...
for (col in colnames(numeric_cols)) {
  boxplot(numeric_cols[[col]], main = col, col = "lightsalmon", border = "maroon", outline = TRUE)
}
...
custom_palette <- colorRampPalette(c("lightsalmon", "white", "maroon"))(200)
...
correlation_analysis <- function(data) {
  ...
  corrplot(cor_matrix, 
           method = "color", 
           col = colorRampPalette(c("lightsalmon", "white", "maroon"))(200), 
           type = "upper", 
           order = "hclust", 
           addCoef.col = "black", 
           tl.col = "black", 
...
ggplot(czyste, aes(x = price)) +
  geom_histogram(aes(y = ..density..), bins = 50, fill = "lightsalmon", color = "black", alpha = 0.7) +
