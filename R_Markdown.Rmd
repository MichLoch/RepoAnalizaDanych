...
#### Instalacja potrzebnych bibliotek i danych
...
#### Wizualizacja brakujących danych za pomocą mapy brakujących danych

```{r visualize-missing-data, echo=TRUE, message=FALSE, warning=FALSE}
# Mapa brakujących danych
vis_miss(apartments)

# UpSet plot dla brakujących danych
library(naniar)
...
```
W Wyniku analizy zbioru danych, możemy zobaczyć, że dane apartments_pl_2024_06 zawierają wiele wartości odstających. Wartości odstające, możemy zaobserwować dla niemal wszystkich zmiennych. Szczególnymi przykładami może być zmienne: Price, collegeDistance oraz Centre Distance. Wartości odstające mogą również mogą wskazywać na obecność droższych apartamentów znajdujących się w atrakcyjnych lokalizacjach.
...
Wykresy przedstawiają analizę rozkładu cen mieszkań przy ocenie czy rozkład jest normalny, jak wygląda struktura danych oraz czy występują wartości odstające. Rozkład normalny jest zaznaczony czerwoną linią. W każdym przypadku rozkład normalny nie pokrywa się z linią niebieską/czarnymi punktami, które prezentują rzeczywiste wartości cen mieszkań i oznacza, że ceny mieszkań nie są rozkładem normalnym. Znaczy to tyle, że nie będą odpowiednie metody statystyczne, które będą zakładać normalność. Pojawiają się wartości odstające. Rozkład jest skośny prawostronnie, więcej mieszkań wchodzi w niższy zakres cen. Większą średnią cen mieszkań zwiększają pojedyncze mieszkania z górnej półki cenowej. Wnioskowanie statystyczne wymaga wykresów rozkładu danych. Jest to konieczne do wykonania oceny rozkładu danych i do wyboru metod analizy danych.
