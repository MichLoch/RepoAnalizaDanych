...
```{r}
shapiro_results <- by(czyste$price, czyste$rooms, shapiro.test)
shapiro_results

# Sprawdzenie homogeniczności wariancji (test Levene’a)
leveneTest(price ~ as.factor(rooms), data = czyste)

# Przeprowadzenie jednoczynnikowej ANOVA dla liczby pokoi
anova_rooms <- aov(price ~ as.factor(rooms), data = czyste)
summary(anova_rooms)

# Jeśli ANOVA jest istotna, wykonujemy testy post hoc (np. Tukey HSD)
TukeyHSD(anova_rooms)

# Przeprowadzenie ANOVA dla miasta
anova_city <- aov(price ~ city, data = czyste)
summary(anova_city)

# Test post hoc dla miasta
TukeyHSD(anova_city)
