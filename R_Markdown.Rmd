...
library(dplyr)
library(knitr)
library(kableExtra)  # Dodajemy kableExtra
library(scales)  # Dodajemy scales dla formatowania walutowego

# Obliczanie podstawowych statystyk opisowych dla price
price_stats <- czyste %>%
  summarise(
    ...
    skewness_price = skewness(price, na.rm = TRUE),  # Skośność
    kurtosis_price = kurtosis(price, na.rm = TRUE)   # Kurtoza
  )

# Formatowanie danych w walucie
price_stats <- price_stats %>%
  mutate(
    mean_price = number(mean_price, big.mark = ".", decimal.mark = ",", accuracy = 0.01),
    median_price = number(median_price, big.mark = ".", decimal.mark = ",", accuracy = 0.01),
    sd_price = number(sd_price, big.mark = ".", decimal.mark = ",", accuracy = 0.01),
    min_price = number(min_price, big.mark = ".", decimal.mark = ",", accuracy = 0.01),
    max_price = number(max_price, big.mark = ".", decimal.mark = ",", accuracy = 0.01),
    range_price = number(range_price, big.mark = ".", decimal.mark = ",", accuracy = 0.01),
    skewness_price = round(skewness_price, 2),
    kurtosis_price = round(kurtosis_price, 2)
  )

# Wyświetlanie tabeli z granicami, tylko dla statystyk dla price
price_stats %>%
  kable(format = "markdown", caption = "Statystyki dla zmiennej price") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), 
                full_width = F, 
                position = "center", 
                font_size = 12) %>%
  add_header_above(c(" " = 1, "Price Stats" = 7))  # Tylko nagłówek dla price stats
...
```{r tai, echo=FALSE}
kbl(tabela1,caption = "Mieszkania w Polsce- ceny w PLN") %>%
    kable_material(c("striped", "hover"))
tab1<-classIntervals(czyste$price,n=10,style="fixed",fixedBreaks=seq(190000,3100000,by=100000))
jenks.tests(tab1)
```
...
###wizualizacja Price Distribution by City
ggplot(czyste, aes(x = city, y = price)) +  
  geom_boxplot(aes(fill = city), alpha = 0.85) +  
  theme_minimal() +  
  labs(title = "Price Distribution by City", x = NULL, y = "Price") +  # Usunięcie etykiet miast
  theme(axis.text.x = element_blank(), axis.ticks.x = element_blank()) +  # Brak tekstu i znaczników na osi X
  scale_y_continuous(labels = scales::label_number(big.mark = ".", decimal.mark = ","))  # Formatowanie ceny
...
Stworzyony wykres typu HeatMap wskazuje na wysoką korelacje zmiennych squareMeter, rooms i poiCount z zmienną price. Zmienna poiCount oznaczające liczbę tzw. punktów zainteresowania (eng. Point of interest)
