...
```{r ,Tabela cena za m^2 echo=TRUE, message=FALSE, warning=FALSE}
...
```{r ,Rozkład cen apartamentów echo=TRUE, message=FALSE, warning=FALSE}
...
```{r ,Mapy Cieplne echo=TRUE, message=FALSE, warning=FALSE}
...
```{r Dystrybucja typów budynków, echo=TRUE, message=FALSE, warning=FALSE}
type_summary <- czyste %>%
  count(type) %>%
  mutate(percentage = n / sum(n) * 100)
...
```{r Wykresy Pudełkowe, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r Wnioskowanie Statystyczne, echo=TRUE, message=FALSE, warning=FALSE}
